import { IconDictionary } from "./definitions";

export const ComedyIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
    >
      <path
        fill="currentColor"
        d="M12 17q1.65 0 2.825-1.175T16 13H8q0 1.65 1.175 2.825T12 17M7 9h4q0-.825-.587-1.412T9 7q-.825 0-1.412.588T7 9m6 0h4q0-.825-.587-1.412T15 7q-.825 0-1.412.588T13 9m-1 13q-1.875 0-3.512-.712t-2.85-1.925q-1.213-1.213-1.925-2.85T3 13V2h18v11q0 1.875-.712 3.513t-1.925 2.85q-1.213 1.212-2.85 1.925T12 22m0-2q2.9 0 4.95-2.05T19 13V4H5v9q0 2.9 2.05 4.95T12 20m0-8"
      />
    </svg>
  );
};
export const CrimeIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 512 512"
    >
      <path
        fill="currentColor"
        d="m390.527 22.85l-72.185 24.5l-20.711 38.71l72.185-24.501zm16.83 6.697L394.76 53.094l18.463-6.268zM121 41v54.324l30-10.181V71h41.664l88.385-30zm172.398 14.816l-72.185 24.502l-20.711 38.711l72.184-24.502zM391 73.38l-30 10.18v121.87l30 9.564zM196.268 88.785l-72.186 24.502l-7.588 14.182l30.496 9.724l28.569-9.697zM343 89.67L175.865 146.4L343 199.69zM99.139 121.754l-.276.094l.192.06zm-30.98 9.197l28.995 40.733l93.37 29.77l-28.997-40.733zm121.948 38.883l28.997 40.732l93.37 29.772l-28.995-40.733zM121 198.18v161.793l30-11.137v-141.09zm191.06 10.539l28.997 40.732l41.777 13.32l31.547-11.709l-8.951-12.574zM169 213.484v128.67l116.621-43.293C281.521 294.4 279 288.468 279 282c0-13.7 11.3-25 25-25c.565 0 1.124.027 1.68.065zm279.635 44.063l-91.873 34.105l-27.06 42.045l91.874-34.105zm-131.711 3.101c7.426 6.439 12.262 14.032 12.057 22.118L343 277.56v-8.598zM304 275c-3.973 0-7 3.027-7 7s3.027 7 7 7s7-3.027 7-7s-3.027-7-7-7m24.637 27.092l-91.873 34.105l-27.06 42.047l91.872-34.105zM391 330.142l-30 11.137v83.834l30 6.024zM208.639 346.64l-83.713 31.076l55.945 11.232l.707-.261zM343 347.96l-128.563 47.726L343 421.5zM80.54 387.162l30.355 36.695l88.02 17.672l-30.356-36.695zm116.034 23.297l30.356 36.695l88.021 17.672l-30.355-36.695zm116.035 23.297l30.356 36.695l88.02 17.672l-30.356-36.695zM121 444.244V487h30v-36.732zm48 9.637V487h164.951zm259.645 3.172l20.35 24.6l3.958-19.72z"
      />
    </svg>
  );
};
export const DocumentaryIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 48 48"
    >
      <g
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeWidth="2"
      >
        <path d="M32 6H22v36h10zm10 0H32v36h10zM10 6l8 1l-3.5 35L6 41z" />
        <path strokeLinecap="round" d="M37 18v-3m-10 3v-3" />
      </g>
    </svg>
  );
};
export const DramaIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 512 512"
    >
      <path
        fill="currentColor"
        d="M418.813 30.625c-21.178 26.27-49.712 50.982-84.125 70.844c-36.778 21.225-75.064 33.62-110.313 38.06a310.317 310.317 0 0 0 6.813 18.25c16.01.277 29.366-.434 36.406-1.5l9.47-1.53l8.436-1.28l.22 10.186a307.48 307.48 0 0 1-1.095 18.72l56.625 8.843c.86-.095 1.713-.15 2.563-.157c11.188-.114 21.44 7.29 24.468 18.593c.657 2.448.922 4.903.845 7.313c5.972-2.075 11.753-4.305 17.28-6.72l9.595-4.188l2.313 10.22a340.211 340.211 0 0 1 7.375 48.062C438.29 247.836 468.438 225.71 493 197.5c-3.22-36.73-16.154-78.04-39.125-117.813a290.509 290.509 0 0 0-2.22-3.78l-27.56 71.374c5.154.762 10.123 3.158 14.092 7.126c9.81 9.807 9.813 25.69 0 35.5c-9.812 9.81-25.722 9.807-35.53 0c-8.86-8.858-9.69-22.68-2.532-32.5l38.938-100.844a322.02 322.02 0 0 0-20.25-25.937zM51.842 118.72c-8.46 17.373-15.76 36.198-21.187 56.436c-14.108 52.617-13.96 103.682-2.812 143.438c13.3-2.605 26.442-3.96 39.312-4.03c1.855-.012 3.688.02 5.53.06c20.857.48 40.98 4.332 59.97 11.5a355.064 355.064 0 0 1-1.656-34.218c0-27.8 3.135-54.377 9-78.937l2.47-10.407l9.655 4.562c29.467 13.98 66.194 23.424 106.28 25.22c5.136-20.05 8.19-39.78 9.408-58.75c-35.198 4.83-75.387 2.766-116.407-8.22c-38.363-10.272-72.314-26.78-99.562-46.656zm230.594 82.218c-1.535 10.452-3.615 21.03-6.218 31.687a312.754 312.754 0 0 0 46-3.97a24.98 24.98 0 0 1-1.532-21.748zM105 201.375l4.156 18.22l-21.594 4.905c8.75 5.174 13.353 15.703 10.594 26c-3.32 12.394-16.045 19.758-28.437 16.438c-12.394-3.32-19.76-16.075-16.44-28.47a23.235 23.235 0 0 1 3.126-6.874l-21.062 4.78l-4.125-18.218l73.78-16.78zm388.594 22.813c-25.53 25.46-55.306 45.445-86.906 60.5c.05 2.397.093 4.8.093 7.218c0 9.188-.354 18.232-1.03 27.125c16.635 1.33 32.045-1.7 45.344-9.374c25.925-14.962 40.608-45.694 42.5-85.47zm-338.844 3c-4.03 19.993-6.33 41.31-6.406 63.593l.125-.342c30.568 10.174 62.622 17.572 95.25 21.375l7.5.875l.718 7.5l5.687 60.125l-18.625 1.75l-2.53-26.75a23.117 23.117 0 0 1-14.845.968c-12.393-3.32-19.76-16.042-16.438-28.436c.285-1.06.647-2.08 1.063-3.063a496.627 496.627 0 0 1-57.406-14.53c2.69 49.62 16.154 94.04 36.094 126.656c22.366 36.588 52.13 57.78 83.968 57.78c31.838.003 61.602-21.19 83.97-57.78c19.536-31.96 32.846-75.244 35.905-123.656a499.132 499.132 0 0 1-48.25 11.656c1.914 4.57 2.415 9.78 1.033 14.938c-3.322 12.394-16.045 19.758-28.438 16.437a23.01 23.01 0 0 1-2.125-.686l-2.5 26.47l-18.594-1.752l5.688-60.125l.72-7.5l7.498-.875c29.245-3.407 57.995-9.717 85.657-18.312v-1.594c0-21.573-2.27-42.23-6.064-61.75C351.132 242.653 313.092 250 272.312 250c-43.59 0-83.986-8.658-117.562-22.813zm-87.5 105.968c-10.87.102-21.995 1.22-33.375 3.313c12.695 31.62 33.117 53.07 59 60c16.9 4.523 34.896 2.536 52.813-5.25c-4.382-13.89-7.874-28.606-10.344-43.97c-21.115-9.623-43.934-14.32-68.094-14.094zm137.5 80.22h130.813c-40.082 44.594-92.623 42.844-130.813 0"
      />
    </svg>
  );
};
export const FamilyIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 2048 2048"
    >
      <path
        fill="currentColor"
        d="M1790 1717q98 48 162 135t81 196h-110q-11-57-41-106t-73-84t-97-56t-112-20q-59 0-112 20t-97 55t-73 85t-41 106h-110q16-108 80-195t163-136q-57-45-88-109t-32-136q0-45 12-87t36-79t57-66t74-49q-27-39-62-69t-76-53t-86-33t-93-12q-80 0-153 31t-127 91q24 65 24 134q0 92-41 173t-115 136q65 33 117 81t90 108t57 128t20 142H896q0-79-30-149t-82-122t-123-83t-149-30q-80 0-149 30t-122 82t-83 123t-30 149H0q0-73 20-141t57-128t89-108t118-82q-74-55-115-136t-41-173q0-79 30-149t82-122t122-83t150-30q85 0 161 36t132 100q26-25 56-45t63-38q-74-55-115-136t-41-173q0-79 30-149t82-122t122-83t150-30q79 0 149 30t122 82t83 123t30 149q0 92-41 173t-115 136q70 37 126 90t95 123q64 0 120 24t99 67t66 98t24 121q0 72-31 136t-89 109M512 1536q53 0 99-20t82-55t55-81t20-100q0-53-20-99t-55-82t-81-55t-100-20q-53 0-99 20t-82 55t-55 81t-20 100q0 53 20 99t55 82t81 55t100 20m384-896q0 53 20 99t55 82t81 55t100 20q53 0 99-20t82-55t55-81t20-100q0-53-20-99t-55-82t-81-55t-100-20q-53 0-99 20t-82 55t-55 81t-20 100m704 630q-42 0-78 16t-64 43t-44 64t-16 79q0 42 16 78t43 64t64 44t79 16q42 0 78-16t64-43t44-64t16-79q0-42-16-78t-43-64t-64-44t-79-16"
      />
    </svg>
  );
};
export const FantasyIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
    >
      <path
        fill="currentColor"
        d="M20 12v7l-3 1l-3-4.67c-.29-.44-1-.19-.92.34L14 23L4 18l.96-5.25C5.56 9.42 8.46 7 11.84 7H13l6-6l-2 6h3l-1.58 2.37C19.36 9.88 20 10.86 20 12"
      />
    </svg>
  );
};
export const HistoryIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 256 256"
    >
      <path
        fill="currentColor"
        d="M216 36h-64a4 4 0 0 0-3.17 1.56l-66.78 86.82l-12.86-12.86a12 12 0 0 0-17 0l-12.67 12.69a12 12 0 0 0 0 17L62.34 164l-30.82 30.83a12 12 0 0 0 0 17l12.68 12.66a12 12 0 0 0 17 0L92 193.67l22.81 22.82a12 12 0 0 0 17 0l12.69-12.7a12 12 0 0 0 0-17L131.62 174l86.82-66.79A4 4 0 0 0 220 104V40a4 4 0 0 0-4-4M55.52 218.83a4 4 0 0 1-5.66 0l-12.69-12.68a4 4 0 0 1 0-5.66L68 169.67L86.34 188Zm83.31-26.36a4 4 0 0 1 0 5.67l-12.7 12.69a4 4 0 0 1-5.66 0l-75.3-75.3a4 4 0 0 1 0-5.66l12.7-12.69a4 4 0 0 1 5.66 0ZM212 102l-86.08 66.22L109.66 152l53.17-53.17a4 4 0 1 0-5.66-5.66L104 146.34l-16.25-16.26L154 44h58Z"
      />
    </svg>
  );
};
export const MysteryIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
    >
      <path
        fill="currentColor"
        d="M11 18q2.9 0 4.95-2.047Q18 13.906 18 11q0-2.9-2.05-4.95T11 4Q8.094 4 6.047 6.05T4 11q0 2.906 2.047 4.953T11 18m0-3.308q-1.96 0-3.476-1.006Q6.008 12.679 5.327 11q.68-1.654 2.197-2.673Q9.04 7.307 11 7.307q1.935 0 3.451 1.02T16.654 11q-.687 1.679-2.203 2.686q-1.516 1.006-3.451 1.006m0-.884q1.471 0 2.717-.75Q14.963 12.31 15.69 11q-.727-1.304-1.973-2.056T11 8.192q-1.477 0-2.733.752Q7.012 9.696 6.285 11q.727 1.31 1.982 2.059q1.256.749 2.733.749m0-1.308q.625 0 1.063-.435q.437-.434.437-1.065q0-.625-.437-1.062T11 9.5q-.63 0-1.065.438Q9.5 10.375 9.5 11q0 .63.435 1.065q.434.435 1.065.435m0 6.5q-1.663 0-3.118-.626q-1.455-.626-2.543-1.713q-1.087-1.088-1.713-2.543Q3 12.663 3 11q0-1.658.626-3.113q.626-1.454 1.713-2.545q1.088-1.09 2.543-1.716Q9.337 3 11 3q1.658 0 3.113.626q1.454.626 2.545 1.716q1.09 1.09 1.716 2.546Q19 9.342 19 11q0 1.523-.535 2.871q-.534 1.348-1.478 2.427L21 20.312l-.694.688l-4.014-4.008q-1.079.945-2.427 1.476Q12.517 19 11 19"
      />
    </svg>
  );
};
export const RomanceIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 256 256"
    >
      <path
        fill="currentColor"
        d="M178 32c-20.65 0-38.73 8.88-50 23.89C116.73 40.88 98.65 32 78 32a62.07 62.07 0 0 0-62 62c0 70 103.79 126.66 108.21 129a8 8 0 0 0 7.58 0C136.21 220.66 240 164 240 94a62.07 62.07 0 0 0-62-62m-50 174.8C109.74 196.16 32 147.69 32 94a46.06 46.06 0 0 1 46-46c19.45 0 35.78 10.36 42.6 27a8 8 0 0 0 14.8 0c6.82-16.67 23.15-27 42.6-27a46.06 46.06 0 0 1 46 46c0 53.61-77.76 102.15-96 112.8"
      />
    </svg>
  );
};
export const ScienceIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 256 256"
    >
      <path
        fill="currentColor"
        d="M150 224a6 6 0 0 1-6 6h-32a6 6 0 0 1 0-12h32a6 6 0 0 1 6 6m-22-114a10 10 0 1 0-10-10a10 10 0 0 0 10 10m93.67 45.4L209.31 211a14 14 0 0 1-22.31 8l-27.79-21H96.82L69 219a14 14 0 0 1-22.34-8l-12.33-55.6a14.06 14.06 0 0 1 2.91-12l29-34.76a121.28 121.28 0 0 1 8.48-36.71c12.72-31.88 35.52-51.88 44.73-59a14 14 0 0 1 17.16 0c9.21 7.12 32 27.12 44.73 59a121.28 121.28 0 0 1 8.48 36.71l29 34.76a14.06 14.06 0 0 1 2.85 12M98.26 186h59.48c21.93-38.46 26.12-75.33 12.43-109.62c-11.95-30-34.35-48.87-40.93-54a2 2 0 0 0-2.48 0c-6.58 5.09-29 24-40.93 54c-13.69 34.29-9.5 71.16 12.43 109.62M87 190.4c-12-21.49-18.9-42.6-20.62-63.19l-19.92 23.87a2 2 0 0 0-.42 1.71l12.37 55.64a2 2 0 0 0 3.2 1.13l.13-.11Zm122.57-39.32l-19.89-23.87c-1.72 20.59-8.6 41.7-20.62 63.19l25.23 19l.13.11a2 2 0 0 0 3.2-1.13L210 152.79a2 2 0 0 0-.46-1.71Z"
      />
    </svg>
  );
};
export const ThrillerIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 512 512"
    >
      <path
        fill="currentColor"
        d="m30.653 470.114l55.34-20.32l84.09-19.62l10.78 18.22l25 15.92l41.34-5.41l27.86 8.41l39.41 1.4l36.71-34.72l46.68 19.3l37.84 9.11l58.628 32.737H17.67zm238.6-202.77l-6 10c10.09 6.1 21.58 6.39 32.88 3.21a92.26 92.26 0 0 0 15.72-6.22c-.31 6.28-.36 16.58-.19 29.29l-18.82-2.87l3.86 115l19.72-.85c.69 9.53 1.45 18.47 2.27 26.44l-10.72 10.14l-30.09-1.07l-25.79-7.77l-3.5-1.06l-3.62.47l-35.2 4.59l-4.89-3.12c8.11-37.88 17.45-123.11 19.51-141.7c.89-7.27 1.226-14.6 1-21.92l9.89-5.34l.7-6.39l-3-6.52h-8.88l-4.65-9.13l-1.83.63a30 30 0 0 0-2.63-3.11l-.08-.08l-24-25.88l2.36-6l-1.18-7.08l-8.51-2.36l-2.07-10l-13.35.87l-30.95-33.39c.53-4.33 3-9.83 6.9-14.16c3.7-4.16 8.26-6.81 11.88-7.54l59 62.41l9.49-6.55l-43.09-89.08c1.73-4 5.71-8.14 10.73-10.83c4.84-2.6 10.11-3.49 13.75-3l45.5 92.27l11-3.53l-16.29-98.92c2.68-2.68 7.66-5.23 13-6a27.14 27.14 0 0 1 3.55-.27h1.17a21.3 21.3 0 0 1 9.93 2.59l15.06 94.32l11.43.35l17.77-80.41c5.89 4.64 19.24 15.77 19.24 15.77l7.25-2.3l-19.62 89.33h.06l-1.44 6.31l-.64 2.88l8.88 5.78c14.3-8.86 28.38-18 40.27-22.17c7.78-2.7 14.2-3.2 19.36-1.17c4.25 1.67 8.26 5.37 11.9 12.43c-29.73 14.51-55.85 34.55-78.25 48.74c-11.87 7.52-22.69 13.34-32 16c-9.31 2.66-16.73 2.22-23.75-2.03m-9.84 65.39v26l11.38 9.15l-4.78 6.2l.28 18.89l8.3 9.15l11.94 9.9l-3.6-107.26l-8.34 5.56l-7.74 22.43h-7.44z"
      />
    </svg>
  );
};
export const WarIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 48 48"
    >
      <g fill="currentColor" fillRule="evenodd" clipRule="evenodd">
        <path d="M9.007 24c-1.104 0-2.021-.9-1.87-1.995C8.07 15.218 13.696 10 20.5 10c4.997 0 9.36 2.815 11.694 7H43a1 1 0 1 1 0 2h-9.886c.352.956.607 1.962.75 3.005c.15 1.095-.767 1.995-1.872 1.995H20.98zm11.937-2H9.159c.92-5.728 5.697-10 11.341-10c5.644 0 10.422 4.273 11.341 10H20.996zM18 32a3 3 0 1 1-6 0a3 3 0 0 1 6 0m-2 0a1 1 0 1 1-2 0a1 1 0 0 1 2 0m8 3a3 3 0 1 0 0-6a3 3 0 0 0 0 6m0-2a1 1 0 1 0 0-2a1 1 0 0 0 0 2m12-1a3 3 0 1 1-6 0a3 3 0 0 1 6 0m-2 0a1 1 0 1 1-2 0a1 1 0 0 1 2 0" />
        <path d="M7.187 26c-2.174 0-3.709 2.006-3.021 3.949l1.397 3.948C6.43 36.347 8.864 38 11.606 38h24.788c2.742 0 5.176-1.653 6.043-4.103l1.397-3.948c.688-1.943-.847-3.949-3.021-3.949zm34.621 2.461c-.197-.257-.54-.461-.995-.461H7.187c-.455 0-.799.204-.995.461a.84.84 0 0 0-.14.82l1.397 3.95C8.013 34.823 9.649 36 11.606 36h24.788c1.957 0 3.593-1.176 4.157-2.77l1.398-3.948a.84.84 0 0 0-.141-.82" />
      </g>
    </svg>
  );
};
export const SpinnerIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style} animate-spin`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 24 24"
    >
      <g fill="currentColor">
        <path
          fillRule="evenodd"
          d="M12 19a7 7 0 1 0 0-14a7 7 0 0 0 0 14m0 3c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10"
          clipRule="evenodd"
          opacity="0.2"
        />
        <path d="M2 12C2 6.477 6.477 2 12 2v3a7 7 0 0 0-7 7z" />
      </g>
    </svg>
  );
};
export const SunIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style} animate-fade animate-duration-300`}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      strokeWidth="1.25"
      stroke="currentColor"
      fill="none"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path d="M12 12m-4 0a4 4 0 1 0 8 0a4 4 0 1 0 -8 0" fill="none" />
      <path d="M3 12h1m8 -9v1m8 8h1m-9 8v1m-6.4 -15.4l.7 .7m12.1 -.7l-.7 .7m0 11.4l.7 .7m-12.1 -.7l-.7 .7" />
    </svg>
  );
};

export const MoonIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style} animate-fade animate-duration-300`}
      xmlns="http://www.w3.org/2000/svg"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      strokeWidth="1.25"
      stroke="currentColor"
      fill="none"
      strokeLinecap="round"
      strokeLinejoin="round"
    >
      <path stroke="none" d="M0 0h24v24H0z" fill="none" />
      <path
        d="M12 3c.132 0 .263 0 .393 0a7.5 7.5 0 0 0 7.92 12.446a9 9 0 1 1 -8.313 -12.454z"
        fill="none"
      />
    </svg>
  );
};

export const MagnifyIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 256 256"
    >
      <path
        fill="currentColor"
        d="m229.66 218.34l-50.07-50.06a88.11 88.11 0 1 0-11.31 11.31l50.06 50.07a8 8 0 0 0 11.32-11.32M40 112a72 72 0 1 1 72 72a72.08 72.08 0 0 1-72-72"
      />
    </svg>
  );
};

export const MenuIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 32 32"
    >
      <path
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="2"
        d="M4 8h24M4 16h24M4 24h24"
      />
    </svg>
  );
};
export const BackIcon = ({ style }: { style?: string }) => {
  return (
    <svg
      className={`${style}`}
      xmlns="http://www.w3.org/2000/svg"
      width="1em"
      height="1em"
      viewBox="0 0 512 512"
    >
      <path
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="32"
        d="m112 160l-64 64l64 64"
      />
      <path
        fill="none"
        stroke="currentColor"
        strokeLinecap="round"
        strokeLinejoin="round"
        strokeWidth="32"
        d="M64 224h294c58.76 0 106 49.33 106 108v20"
      />
    </svg>
  );
};

export const iconDictionary: IconDictionary = {
  Comedy: <ComedyIcon />,
  Crime: <CrimeIcon />,
  Drama: <DramaIcon />,
  Documentary: <DocumentaryIcon />,
  Family: <FamilyIcon />,
  Fantasy: <FantasyIcon />,
  History: <HistoryIcon />,
  Mystery: <MysteryIcon />,
  Romance: <RomanceIcon />,
  "Science Fiction": <ScienceIcon />,
  Thriller: <ThrillerIcon />,
  War: <WarIcon />,
};
